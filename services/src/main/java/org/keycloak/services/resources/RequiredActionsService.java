...
public class RequiredActionsService {
    ...
    public Response updateTotp(final MultivaluedMap<String, String> formData) {
        ...
        Forms forms = Flows.forms(realm, request, uriInfo).setUser(user);
        if (Validation.isEmpty(totp)) {
            return forms.setError(Messages.MISSING_TOTP).createResponse(RequiredAction.CONFIGURE_TOTP);
        } else if (!new TimeBasedOTP().validate(totp, totpSecret.getBytes())) {
            return forms.setError(Messages.INVALID_TOTP).createResponse(RequiredAction.CONFIGURE_TOTP);
        }
    ...
    public Response updatePassword(final MultivaluedMap<String, String> formData) {
        ...
        if (Validation.isEmpty(passwordNew)) {
            return forms.setError(Messages.MISSING_PASSWORD).createResponse(RequiredAction.UPDATE_PASSWORD);
        } else if (!passwordNew.equals(passwordConfirm)) {
            return forms.setError(Messages.NOTMATCH_PASSWORD).createResponse(RequiredAction.UPDATE_PASSWORD);
        }

        String error = realm.getPasswordPolicy().validate(passwordNew);
        if (error != null) {
            return forms.setError(error).createResponse(RequiredAction.UPDATE_PASSWORD);
        }
